...
> - [x] PiP entry behavior tuned toward fullscreen/video-only capture path.
...
- First-play startup latency mitigation applied (non-blocking prewarm); continue device validation for backend cold-start variance.
