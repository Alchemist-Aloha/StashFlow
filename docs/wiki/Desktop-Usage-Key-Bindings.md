...
| `F` | Toggle fullscreen |
| `P` | Toggle Picture-in-Picture (Android and Linux) |
...
While the Linux PiP window is active, drag the video to move it, double-click it or press `Esc` to exit PiP, and hover for previous, play/pause, next, seek, and exit-PiP controls.
