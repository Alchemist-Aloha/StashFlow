...
## 2024-05-19 - Defer Riverpod lookups in highly active Scroll Listeners
**Learning:** In Flutter, `NotificationListener<ScrollNotification>` callbacks can fire 60 to 120 times per second during active scrolling. Unconditionally querying a Riverpod provider (`ref.read(...)`) at the beginning of these callbacks incurs small but repetitive overhead, increasing GC pressure and potentially contributing to frame drops, even if the subsequent logic short-circuits.
**Action:** Always hoist invariant state outside the scroll callback if it doesn't change during scroll. If a read *must* happen inside the callback, strictly defer `ref.read` (or similar Context lookups) until *after* any early-return or throttling conditions (e.g., index change checks) to ensure it is only executed when its value is genuinely required.
