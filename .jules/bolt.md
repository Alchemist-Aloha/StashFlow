...
## 2026-05-06 - [Avoid Recomputing Layout Dimensions Inside Scroll Listeners]
**Learning:** In Flutter, `NotificationListener<ScrollNotification>` callbacks fire frequently during scrolling (up to 120 times per second). Querying `ref.read(mediaHeadersProvider)` or computing spacing boundaries dynamically within the `onNotification` callback triggers unnecessary inherited widget lookups on every frame tick, degrading scroll performance. Wait, using `ref.read` in `build` is an anti-pattern.
**Action:** Use `ref.read` sparingly within the actual logic scope where it's needed, but wrap it *after* short-circuit return checks (like `if (visibleIndex == lastVisibleIndex) return;`) so it only fires when new data needs loading. Never hoist `ref.read` to the `build` method, as that defeats reactivity.
