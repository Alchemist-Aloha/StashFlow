...
## 2024-05-19 - Throttle Scroll Pagination
**Learning:** In Flutter, `NotificationListener<ScrollNotification>` triggers on every frame during a scroll. If you bind pagination callbacks like `onFetchNextPage` to scroll metrics (e.g. `metrics.pixels >= maxScrollExtent - threshold`), the function can be fired up to 60-120 times per second until the new data is loaded and the scroll extent expands, leading to massive redundant O(N) calls, even if the underlying provider ignores subsequent hits.
**Action:** Always throttle pagination events on scroll listeners. A simple `DateTime` comparison limiting calls to once per ~500ms prevents this spam without sacrificing UI responsiveness. Also remember to check `pubspec.lock` modifications caused by automated tooling and revert them.
