...
## 2024-11-20 - Hoist stride calculation and delay provider reads in ListPageScaffold scroll listener
**Learning:** In Flutter, operations executed inside `NotificationListener<ScrollNotification>` run on almost every frame during scrolling. Doing O(N) or even simple inherited widget lookups like `ref.read` and calculating layout math in this tight loop causes noticeable frame drops.
**Action:** Always extract layout calculations (like `stride`, grid sizes, visible index formulas) and inherited lookups OUTSIDE the `onNotification` callback, ideally to the top of `build` or into cached variables. Only do fast math and `if (value == lastValue) return;` throttling inside the listener.
