...
class SceneScrapeNotifier {
  ...
  Future<List<Scraper>> listAvailableScrapers({required List<String> types}) async {
    final repo = ref.read(sceneRepositoryProvider);
    return repo.listScrapers(types: types);
  }

  Future<List<ScrapedScene>> scrapeScene({required String scraperId, required String sceneId}) async {
    final repo = ref.read(sceneRepositoryProvider);
    return repo.scrapeSingleScene(scraperId: scraperId, sceneId: sceneId);
  }

  Future<void> saveScraped({required String sceneId, required ScrapedScene scraped, bool merge = false, List<String>? performerIds, List<String>? tagIds}) async {
    final repo = ref.read(sceneRepositoryProvider);
    await repo.saveScrapedScene(sceneId: sceneId, scraped: scraped, mergeValues: merge, performerIds: performerIds, tagIds: tagIds);
  }
